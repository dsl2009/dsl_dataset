...
from dsl_data.static_data import clolors
import os
...
def roi_align(boxes,feature_maps,cfg):
    ...
    return pooled
def get_color_rate(hsv, low, upper, total, w , h, is_black=False):
    y_mask = cv2.inRange(hsv, low, upper)
    y_mask = y_mask / 255
    if is_black:
        y_zhisu = (np.sum(y_mask)-total) / (w * h - total)
    else:
        y_zhisu = np.sum(y_mask) / (w * h - total)
    return y_zhisu

def get_green_rate(mask, image, is_show=False):
    ...
    k = np.argmax(d)
    tmig = np.zeros(shape=im.shape, dtype=np.uint8)
    cv2.drawContours(tmig, contours, k, (255, 255,255), -1)
    tmig = tmig/255

    new_mask = org*tmig

    ss = np.sum(tmig, axis=2)
    total = len(np.where(ss == 0)[0])
    
    cv2.imwrite('tmp.jpg',new_mask)
    w, h ,d  = new_mask.shape
    nn = cv2.imread('tmp.jpg')
    plt.subplot(221)

    hsv = cv2.cvtColor(nn, cv2.COLOR_BGR2HSV)
    result = []
    for ii, x in enumerate(clolors):
        if ii == 0:
            y_zhisu = get_color_rate(hsv,np.asarray(x['low']), np.asarray(x['upper']), total, w ,h,is_black=False)
        else:
            y_zhisu = get_color_rate(hsv, np.asarray(x['low']), np.asarray(x['upper']), total, w, h, is_black=False)

        result.append(y_zhisu)
    result.append(max([1-sum(result),0]))
    return result



def get_right_leaf(mask, image):
    ...
    img_pth = image.replace('step3','step3_mask')
    dr = img_pth.split('/')
    ndr = '/'.join(dr[:-1])
    if not os.path.exists(ndr):
        os.makedirs(ndr)
    cv2.imwrite(img_pth, new_mask)

def get_roate(mask, image):
    print(image)
    org = cv2.imread(image)
    im = cv2.imread(mask)
    imgray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    ...
    y3, x3 = box[2]
    y4, x4 = box[3]
    w = np.sqrt((y1-y2)**2+(x1-x2)**2)
    h = np.sqrt((y2 - y3) ** 2 + (x2 - x3) ** 2)
    if w>h:
        jd = np.arctan((y1-y2)/(x1-x2))/np.pi*180+90
    else:
        jd = 90 - np.arctan((y1 - y2) / (x1 - x2)) / np.pi * 180
